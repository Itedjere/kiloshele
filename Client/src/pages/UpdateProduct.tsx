...
export default function UpdateProduct() {
  ...
  const [updateProduct] = useMutation(UPDATE_PRODUCT, {
    update(cache, { data }) {
      ...
      const newCategory = data.updateProduct.category;
  ...
  useEffect(() => {
    if (productData?.productOne) {
      const product = productData.productOne;
      reset({
        name: product.name,
        sku: product.sku,
      ...
      //   Set Offer Type
      setOfferType(product.type === "PRODUCT" ? "Product" : "Service");
      //   Set other fees if its service and there are other services
  ...
  const onSubmit: SubmitHandler<AddProductsFormDataType> = async (formData) => {
    ...
    try {
      ...
      if (data?.updateProduct) {
        ...
        toast.success(
          `${
            data.updateProduct.type === "PRODUCT" ? "Product" : "Service"
  ...
  if (productError)
    return (
      <ServerError
        ...
        url={`${import.meta.env.VITE_CLIENT_URL}/all-products`}
      />
    );
                        ...
                        <Accordion.Body>
                          <div className="row">
                            <div className="col-sm-12 col-lg-4 mb-3">
                              ...
                              <small>
                                Example: 1 plate of rice with turkey
                              </small>
                              {errors.name && (
                        ...
                        <Accordion.Body>
                          <div className="row">
                            <div className="col-12">
                              <p>
                                <small>
                                  You can upload up to 3 pictures, each less
                                  than 3MB. Only Images / Pictures are allowed.
                                </small>
                              </p>
                              <FileDropzone
                                files={files}
